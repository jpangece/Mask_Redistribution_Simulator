...
float eff_rt_calculator(float sus_num, float rec_num, float cur_msk_num,float req_msk_num, float radius)
{
    ...
    if (cur_msk_num < req_msk_num*mask_use_rt&&msk_num_for_sus<sus_num)
    {
        ...
        eff_rt = 1 / (total_area * (sus_num - msk_num_for_sus)) * ((sus_num - msk_num_for_sus) - 1 - total_area / unit_area > 0 ? total_area / unit_area * (total_area + unit_area) / 2/sus_num + ((sus_num - msk_num_for_sus) - 1 - total_area / unit_area) * total_area/sus_num : ((sus_num - msk_num_for_sus) / 2) * ((sus_num - msk_num_for_sus) - 1) * unit_area)/sus_num;
    }
    //        Equivalently
...
{
	return city.cur_msk_num + threshold_msk_percent * city.total_num;
}