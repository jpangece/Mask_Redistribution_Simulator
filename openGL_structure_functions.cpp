...
void RefreshDatas()
{
    ...
    auto time_offset = current_virtual_time - start_virtual_time
    int current_hour = time_offset / 3600;
    is_hour_changed = current_hour != pre_hour;
    pre_hour = current_hour;

    // Compute the refreshing period of function plotting
    int current_cycle_pos = time_offset / inf_refresh_cycle;
    need_refresh_inf = current_cycle_pos != inf_pre_cycle_pos;
    inf_pre_cycle_pos = current_cycle_pos;

    auto sub_virtual_time1 = time_offset % (hours_per_trans1 * 3600);
    auto sub_virtual_time2 = time_offset % (hours_per_trans2 * 3600);
    ...
    for (int i = 0; i < city_count; ++i)
    {
        ...
        if (is_hour_changed)
        {
            ...
            city.current_hour_mask_num = (int)compute_show_mask_num(city);
        }
...
void Show()
{
    ...
    if (is_inputed_values)
    {
        ...
        else
        {
            char end_str[100];
            sprintf_s(
                end_str, "%d ten-thousand(s) of citizens are infected in %s. The simulation ends.",
                threshold_infection_percent, city_infos[end_check_result].name.c_str());
...
int end_check()
{
    for(int i=0; i<city_count; ++i)
    {
        float threshold = (float)threshold_infection_percent / 10000 * city_infos[i].total_num
        if(city_infos[i].inf_num >= threshold)
        {
            return i;
        }
    }
