...
void DrawString2(std::string str, int x_offset, int y_offset,
         float angle, void *font = GLUT_STROKE_ROMAN)
{
    glColor3f(0.0f, 0.0f, 0.0f);
    glLoadIdentity();
...
void DrawInputUI()
{
    ...
    DrawString("Mask Distribution Simulator for Hubei Province during COVID-19",
        280, 70, GLUT_BITMAP_HELVETICA_18);
    DrawString("Please enter the mask production capacity (pieces per day) of:",
        290, 170 + base_y, GLUT_BITMAP_HELVETICA_18);
    ...
    DrawString("Example: input \"10\" to set that the simulation ends "
        "when the infection ratio reaches 0.001 in any one of the cities",
        290, 420 + base_y, GLUT_BITMAP_HELVETICA_12);
    DrawString("scale for reference: for Wuhan,9785392(population) * 10 * 1/10000 = 9785",
        290, 440 + base_y, GLUT_BITMAP_HELVETICA_12);
    ...
    glColor3f(threshold_infection_input_color,
          threshold_infection_input_color,
          threshold_infection_input_color);
...
void DrawCityDatas()
{
    ...
    for (int i = 0; i < city_count; i++)
    {
        ...
        _itoa_s(compute_show_mask_num(city), cur_msk_num_str, 10);
#else
        _itoa(city.total_num, total_num_str, 10);
        _itoa(city.inf_num, inf_num_str, 10);
        _itoa(city.rec_num, rec_num_str, 10);
        _itoa(compute_show_mask_num(city), cur_msk_num_str, 10);
#endif // WINDOWS
...
// Compute the max infected number in history
...
// Compute the max mask number in history
float MaxHistoryMaskNums(const CITY& city)
{
    float max_val = 0;
    for (int i = 0; i < city.history_msk_num.size(); ++i)
    {
        auto inf_num = city.history_msk_num[i];
        max_val = max(max_val, inf_num);
    }
    return max_val;
}

// Compute the max value of the y axis in dialog's graph
float ComputeMaskNumsYAxisUpper(const CITY& city)
{
    int max_val = MaxHistoryMaskNums(city);
    int result = 1;
    int val = max_val;
...
void SetInfectFunctionColor()
{
    glColor3f(0.9f, 0.0f, 0.0f);
}

void SetMaskFunctionColor()
{
    glColor3f(0.0f, 0.0f, 0.9f);
}

void DrawInfectHistoryFunctionCurve(
    const _CITY &city_info, int y_origin, int y_end, int x_origin, int x_end)
{
    auto max_inf_num = MaxHistoryInfNums(city_info);
    auto upper_limit = 0.1f;
    const int y_axis_label_count = 5;
    int y_label_space = (y_origin-y_end) / y_axis_label_count;

    // Mark the unit of y axis (graph of infected number)
...
void DrawMaskHistoryFunctionCurve(const _CITY &city_info, int y_origin, int y_end, int x_origin, int x_end)
{
    ...
    for (int i = 0; i < history_inf_cache_count && i < city_info.history_msk_num.size(); i++)
    {
        // Compute infect people percent
        float percent = city_info.history_msk_num[i] / upper_limit;
        int y = y_origin - (y_origin - y_end) * percent;
        glVertex2f(ParseOpenGLX(i + x_origin), ParseOpenGLY(y));
    }
...
void ShowDialog()
{
    if (dialog_city_index >= 0)
    {
        ...
        glRectf(ParseOpenGLX(x1 + 420), ParseOpenGLY(y1 + 40),
            ParseOpenGLX(x1 + 460), ParseOpenGLY(y1 + 50));
        ...
        // y_axis of mask number
        SetMaskFunctionColor();
        glBegin(GL_LINES);
        glVertex2f(ParseOpenGLX(x_end), ParseOpenGLY(y_origin));
        glVertex2f(ParseOpenGLX(x_end), ParseOpenGLY(y_end));
        glVertex2f(ParseOpenGLX(x_end), ParseOpenGLY(y_end));
        glVertex2f(ParseOpenGLX(x_end + 10), ParseOpenGLY(y_end + 10));
        glVertex2f(ParseOpenGLX(x_end), ParseOpenGLY(y_end));
        glVertex2f(ParseOpenGLX(x_end - 10), ParseOpenGLY(y_end + 10));
        glEnd();
        ...
        DrawString("Mask", x_end, y_end - 10, GLUT_BITMAP_HELVETICA_12);
        DrawString("0", x_origin - 10, y_origin, GLUT_BITMAP_HELVETICA_12);
        DrawString("Time", x_end, y_origin + 10, GLUT_BITMAP_HELVETICA_12);
        DrawMaskHistoryFunctionCurve(city_info, y_origin, y_end, x_origin, x_end);
        DrawInfectHistoryFunctionCurve(city_info, y_origin, y_end, x_origin, x_end);

        //text
        char inf_num_str[128];
        char mask_num_str[128];
#ifdef _WINDOWS
        _itoa_s(city_info.inf_num, inf_num_str, 10);
        _itoa_s(compute_show_mask_num(city_info), mask_num_str, 10);
#else
        _itoa(city_info.inf_num, inf_num_str, 10);
        _itoa(compute_show_mask_num(city_info), mask_num_str, 10);
#endif
        char rec_rt_str[128];
...
void ShowInfo()
{
    if(is_info_shown==1)
    {
        ...
        DrawString("Welcome!",
            info_start_x + info_left_space, info_start_y + 2 * linespace, GLUT_BITMAP_HELVETICA_12);
        DrawString("This program tends to simulate a scientific mask redistribution in Hubei Province, China, during the Covid-19.",
            info_start_x + info_left_space, info_start_y + 3 * linespace, GLUT_BITMAP_HELVETICA_12);
        DrawString("Note that in our model, Xiantao is deemed as the major mask production city with a higher production capacity",
            info_start_x + info_left_space, info_start_y + 4.5 * linespace, GLUT_BITMAP_HELVETICA_12);
        DrawString("(rich in fabric production). Other cities have a lower production capacity as there are less mask production.",
            info_start_x + info_left_space, info_start_y + 5.5 * linespace, GLUT_BITMAP_HELVETICA_12);
        DrawString("(Setting the mask production capacities and tolerable infected ratio will lead you to the next frame)",
            info_start_x + info_left_space,info_start_y + 7 * linespace, GLUT_BITMAP_HELVETICA_12);
        DrawString("-----------------------------------------------------------------------------",
            info_start_x + info_left_space,info_start_y + 8 * linespace, GLUT_BITMAP_HELVETICA_12);
        DrawString("You can see a message box on the upper left, the map on the left and an information list on the right:",
            info_start_x + info_left_space, info_start_y + 9 * linespace, GLUT_BITMAP_HELVETICA_12);
        DrawString("    -Message box tells the end of simulation",
            info_start_x + info_left_space, info_start_y + 10 * linespace, GLUT_BITMAP_HELVETICA_12);
        DrawString("    -Red road indicates the mask transportation from the major production city (every 6 hrs)",
            info_start_x + info_left_space, info_start_y + 11 * linespace, GLUT_BITMAP_HELVETICA_12);
        DrawString("     Yellow road indicates the mask transportation between some non-major cities (every 2 hrs)",
            info_start_x + info_left_space, info_start_y + 12 * linespace, GLUT_BITMAP_HELVETICA_12);
        DrawString("    -Information list records the immediate data of different cities.",
            linfo_start_x + info_left_space, info_start_y + 13 * linespace, GLUT_BITMAP_HELVETICA_12);
        DrawString("More features you might like to know:",
            info_start_x + info_left_space, info_start_y + 14.5 * linespace, GLUT_BITMAP_HELVETICA_12);
        DrawString("    -click the cities on map to inspect the trending of the infected number",
            info_start_x + info_left_space, info_start_y + 15.5 * linespace, GLUT_BITMAP_HELVETICA_12);
        DrawString("    -judge the seriousness of infection by the color of cities on the map",
            info_start_x + info_left_space,info_start_y + 16.5 * linespace, GLUT_BITMAP_HELVETICA_12);
        DrawString("Enjoy it!",
            info_start_x + info_left_space, info_start_y + 18 * linespace, GLUT_BITMAP_HELVETICA_12);
    }
}
