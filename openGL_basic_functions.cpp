...
void DrawInfectHistoryFunctionCurve(const _CITY &city_info, int y_origin, int y_end, int x_origin, int x_end)
{
	auto max_inf_num = MaxHistoryInfNums(city_info);
	auto upper_limit = 0.1f;//min(int(100.0f * (max_inf_num / city_info.total_num)) + 1, 100);
	...
	for (int i = 0; i < y_axis_label_count; ++i)
	{
		...
		DrawString(y_description, x_origin - 60, y_origin - y_label_space * (i + 1) + 10, GLUT_BITMAP_HELVETICA_12);
	}
...
void DrawMaskHistoryFunctionCurve(const _CITY &city_info, int y_origin, int y_end, int x_origin, int x_end)
{
	...
	for (int i = 0; i < y_axis_label_count; ++i)
	{
		...
		DrawString(y_description, x_end + 10, y_origin - y_label_space * (i + 1) + 10, GLUT_BITMAP_HELVETICA_12);
	}
...
void ShowDialog()
{
    if (dialog_city_index >= 0)
    {
		...
		SetInfectFunctionColor();
		glRectf(ParseOpenGLX(x1 + 420), ParseOpenGLY(y1 + 20), ParseOpenGLX(x1 + 460), ParseOpenGLY(y1 + 30));
		DrawString(std::string("Mask:"), x1 + 380, y1 + 50, GLUT_BITMAP_HELVETICA_10);
		SetMaskFunctionColor();
		glRectf(ParseOpenGLX(x1 + 420), ParseOpenGLY(y1 + 40), ParseOpenGLX(x1 + 460), ParseOpenGLY(y1 + 50));
		...
		DrawString(std::string("mask number:") + mask_num_str, x_origin, y_origin + 70, GLUT_BITMAP_HELVETICA_18);
		DrawString(std::string("Recovery Rate:") + rec_rt_str, x_origin, y_origin + 100, GLUT_BITMAP_HELVETICA_18);
    }
}
