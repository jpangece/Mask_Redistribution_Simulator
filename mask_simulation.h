...
typedef struct CITY
{
    ...
    / Effective Contact Rate:
      eff_rt = a + b * (1 - e ^ ( -(sus_num + inf_num + rec_num) / (radius * radius * cur_msk_num) ))
    / Recovery Rate:
      rec_rt = 1 / (15 + 15 * (1 - e ^ (-inf_num / hspt_num)
...
// Color of the selected input box
const float select_input_color = 0.8;

// Color of the normal input box
...
// Input: the pointer of system
// Changes: sys1 -> city[ALL]
//     -> eff_rt (effective contact rate) & rec_rt (recovery rate) updated
...
// Input: the pointer of system
// Changes: sys1 -> city[ALL]
//     -> sus_num (number of suspected people) & inf_num (...) & rec_num (...) updated
...
// Screen coordinate -> opengl coordinate (x axis)
GLfloat ParseOpenGLX(int x);

// Screen coordinate -> opengl coordinate (y axis)
...
// Mouse-triggered-event: input values
void MouseFuncInputValues(int button, int state, int x, int y);

// Mouse-triggered-event: pop-up dialog
